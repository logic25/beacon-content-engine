...
export default function DigestGenerator({ onInsert }: { onInsert?: (body: string) => void }) {
  ...
  const generateDigest = () => {
    ...
    setTimeout(() => {
      ...
      const body = `# Beacon Weekly — ${dateRange}
...
${recentDocs.length > 0
  ? recentDocs.map((d) => `- **${d.title}** — ${d.category}`).join("\n")
...
## 📰 Published Content
${recentContent.map((c) => `- **${c.title}** — ${c.views.toLocaleString()} views, ${c.shares} shares`).join("\n")}
...
---
*Auto-generated by Beacon's Digest Engine · [Edit before sending]*`;

      setGeneratedBody(body);
      setGenerating(false);
      toast({ title: "Digest generated! ✨", description: "Review and edit before sending." });
    }, 2000);
  };
